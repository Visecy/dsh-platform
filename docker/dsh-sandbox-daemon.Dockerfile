...
# Uses the full Debian bookworm base with common dev/debug tools so agents do
# not lose git/curl/wget/etc. and operators can inspect the pod comfortably.
...
RUN apt-get update && apt-get install -y --no-install-recommends \
      tini procps util-linux ripgrep git curl wget ca-certificates \
      vim jq unzip rsync python3 build-essential \
    && rm -rf /var/lib/apt/lists/*
WORKDIR /app
COPY --from=builder /app/node_modules ./node_modules
COPY packages/sandbox-daemon/package.json ./
COPY packages/sandbox-daemon/src ./src
# node:24-bookworm already has uid 1000 (the node user); reuse it
